...
from scrape.notify import find_newly_available_dates, find_newly_blocked_dates
...
def test_finds_date_that_flipped_from_unavailable_to_available():
    old_days = {"2026-08-01": "unavailable", "2026-08-02": "available"}
    new_days = [
        {"date": "2026-08-01", "status": "available"},
        {"date": "2026-08-02", "status": "available"},
    ]
    result = find_newly_available_dates(old_days, new_days)
    assert [d["date"] for d in result] == ["2026-08-01"]


def test_ignores_dates_still_unavailable():
    old_days = {"2026-08-01": "unavailable"}
    new_days = [{"date": "2026-08-01", "status": "unavailable"}]
    assert find_newly_available_dates(old_days, new_days) == []


def test_ignores_brand_new_date_that_appears_already_available():
    old_days: dict[str, str] = {}
    new_days = [{"date": "2026-09-01", "status": "available"}]
    assert find_newly_available_dates(old_days, new_days) == []
